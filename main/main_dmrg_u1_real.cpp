/**
 * main/main_dmrg_u1_real.cpp
 *
 * Test driver for DMRG<double, U1Symmetry> on a 1D Heisenberg chain.
 *
 * Usage:
 *   ./dmrg_u1_real -lx <L> -d <D> -sec <Sz_sector> -niter <niter>
 *                  [-dr <D_read>] [-exci <n_exci>] [-psize <n_threads>]
 *
 * Example (spin-1/2 chain L=8, D=64, Sz=0 sector, 10 sweeps):
 *   ./dmrg_u1_real -lx 8 -d 64 -sec 0 -niter 10
 */
...
DMRG<double, U1Symmetry>* chain_ptr = nullptr;
...
int main(int argc, char** argv) {
  ...
  cout << "U1 DMRG (real): lx=" << lx << " ly=" << ly << " D=" << bondd
  ...
  DMRG<double, U1Symmetry> chain(ly, lx, bondd, physpn, sec, exci);
  chain_ptr = &chain;
