/**
 * main/main_dmrg_su2_complex.cpp
 *
 * Test driver for DMRG<dcmplex, SU2Symmetry> on a 1D Heisenberg chain.
 * SU2 symmetry labels blocks by total spin J (stored as 2J).
 *
 * Usage:
 *   ./dmrg_su2_complex -lx <L> -d <D> -sec <2J> -niter <niter>
 *                      [-dr <D_read>] [-exci <n_exci>] [-psize <n_threads>]
 *
 * Example (spin-1/2 chain L=8, D=64, singlet sector 2J=0, 10 sweeps):
 *   ./dmrg_su2_complex -lx 8 -d 64 -sec 0 -niter 10
 */
...
DMRG<dcmplex, SU2Symmetry>* chain_ptr = nullptr;
...
int main(int argc, char** argv) {
  ...
  cout << "SU2 DMRG: lx=" << lx << " ly=" << ly << " D=" << bondd
       << " 2J=" << sec << " exci=" << exci << " niter=" << niter
       << " threads=" << psize << endl;

  // For SU2: phdim = 2*S + 1 (number of m_J states per site)
  // For spin-1/2: phdim = 2, physpn = 1 (2*S=1)
  int phspn = 1;

  DMRG<dcmplex, SU2Symmetry> chain(ly, lx, bondd, phspn, sec, exci);
  chain_ptr = &chain;
